...
ranker/league.py. The only value input is `points_3yr` from `pool.json` — three-year
points in this league's scoring (see ranker/pool.py for why the provider's 3D value is
deliberately unused). The method, in one breath: how many players start at each position
is an *outcome* of how the league drafts, so replacement levels are found as a fixed
point of an optimal-drafter draft simulation (ranker/sim.py), roster value and the
replacement measurement live in ranker/value.py, and `draft.json` — the live board — is
the simulation's starting state, not a filter (ranker/board.py).

The output's headline `vor` is 3-year points minus the converged marginal-starter level:
