    ...
    return {
        ...
        "my_next_picks": {
            "note": (
                ...
                "survives and the ordinary policy used otherwise. `take` is the EV choice "
                "conditional on being available; `deterministic_fallback` is emitted when "
                "the noiseless example removes that choice first. Its take stands "
                "when a candidate's full-horizon edge over the ordinary policy clears twice "
                "its standard error. When the recommended player survives the noiseless "
